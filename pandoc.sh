...
for folder in copy-src/*/; do
  ...
  echo "  Creating '$lang' HTML document."
  pandoc $(find "$folder" -type f | sort) --template="web-template.html"\
    -o "./docs/index-$lang.html" --metadata title="Know Your Rights!" -c "main.css"
done

echo "Creating default HTML document."
